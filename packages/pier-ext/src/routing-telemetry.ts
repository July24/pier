/**
 * Phase 0 routing telemetry (RFC docs/rfc-jev-role-routing.md §8): observation only, zero behavior
 * change. Three row kinds answer the decision questions before the grants collapse (phase 1) and the
 * jev router (phase 2) are built — spawn profile, deny hits, axis usage.
 *
 * Privacy contract: task text NEVER enters telemetry — only taskSha8 (correlatable, not reversible)
 * and tool/role names. Planners are pure and sessionId-free; the central append (index.ts) stamps the
 * session id once, and every append is best-effort.
 */
...
type SpawnProfileRecord = {
  ...
  role: string;
  /** Model-suggested extras from the spawn `allowed_tools` param. */
  allowedTools: string[];
...
/** sessionId-free: the central append stamps it into RoutingTelemetryRow. */
...
/**
 * Fold role-file texts into the axis-usage summary. A malformed file counts as invalid and never
 * fails the scan. Shapes are mirrored, not imported, so telemetry cannot break on schema drift.
 */
export function scanRoleAxisUsage(opts: {
  ...
  for (const file of opts.files) {
    ...
    // {"*":"allow"}) and unknownTools defaults to deny, so a minimal role still counts as parsed.
