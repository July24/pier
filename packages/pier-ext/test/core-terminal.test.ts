/**
 * 档1 core/terminal 插件接线：surface 服务注入 + 六工具注册 + GC 槽回填 + ledger 墓碑。
...
const TOOL_NAMES = ['terminal_open', 'terminal_send', 'terminal_read', 'terminal_signal', 'terminal_close', 'terminal_list'];

test('core/terminal：surface 挂载六工具 + GC 槽回填 + open 链路', async () => {
  const pi = fakePi();
  ...
  for (const n of TOOL_NAMES) assert.ok(pi.tools.has(n), `${n} 应注册`);
  assert.equal(deps.state.activePaneIds().size, 0, '初始无终端');

  const open = await pi.tools.get('terminal_open')?.execute?.(null, {}, undefined, undefined, { cwd: 'F:/w' }) as {
  ...
  const list = await pi.tools.get('terminal_list')?.execute?.(null, {}) as { content: Array<{ text: string }> };
...
test('core/terminal：ledger.disposeKey(本文件) → 工具墓碑 inert（hmr 补偿路径）', async () => {
  ...
  const r = await pi.tools.get('terminal_list')?.execute?.(null, {}) as { content: Array<{ text: string }> };
