/**
 * herdr-client: env detection, Noop contract, the NDJSON socket protocol (envelope decoding, error
 * mapping, batching) and the A6 wire contract — every request pier really sends is replayed against
 * the parameter table from `herdr api schema` (fixtures/herdr-contract.json): method exists, no
 * unknown fields, required fields present, no undefined values. After a herdr upgrade regenerate the
 * fixture, so drift shows up here instead of in the user's pane.
 */
...
const clientFor = (server: FakeHerdrServer): HerdrClient =>
  new HerdrClient({ socketPath: server.socketPath, paneId: 'p-self', workspaceId: 'w1', tabId: 't1' });

/** Real-socket case: `handler` answers the request, `setup` tweaks the server before it listens. */
function rpcTest(
  name: string,
  handler: RpcHandler,
...
rpcTest('HerdrClient: pane.list maps fields; connection roundtrip', (method) => (method === 'pane.list'
...
rpcTest('HerdrClient: listAgents maps agent_session.value', () => ({ agents: [{
  pane_id: 'p2',
  agent: 'pi',
...
rpcTest('HerdrClient: splitPane extracts a nested pane_id via findIdIn', (method) => (method === 'pane.split' ? { created: { pane_id: 'p-new' } } : {}), async (client, server) => {
...
rpcTest('HerdrClient: splitPane 转发 ratio；pane.layout 解析真矩形', (method) => {
...
rpcTest('HerdrClient: splitPane throws when the response carries no pane_id', () => ({ ok: true }), async (client) => {
  await assert.rejects(() => client.splitPane({}), /no pane_id/);
});

rpcTest('HerdrClient: createTab requires tab_id; sendPaneText appends CR', (method) => (method === 'tab.create' ? { tab_id: 't9', root: { pane_id: 'p9' } } : {}), async (client, server) => {
...
rpcTest('HerdrClient: tabList drops tabs without tab_id; tabClose / closePane', (method) => (method === 'tab.list' ? {
...
rpcTest('HerdrClient: exportLayout reads nested layout envelope; failure → null', () => ({ layout: { tab_id: 't1', zoomed: true, root: { type: 'pane' } } }), async (client, server) => {
  ...
  const missing = new HerdrClient({
    socketPath: join(server.socketPath, 'no-such.sock'),
    paneId: 'p',
    workspaceId: 'w',
    tabId: 't',
  });
...
rpcTest('HerdrClient: waitAgent maps timeout errors to null; returns agent_status on success', () => ({ agent: { agent_status: 'idle' } }), async (client) => {
...
rpcTest('HerdrClient: readPane unwraps read envelope; waitForOutput on a match', (method) => {
