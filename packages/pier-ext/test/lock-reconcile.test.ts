/**
 * Write locks (lock-core), todo↔subagent reconciliation (reconcile-core) and progress badges
 * (progress-core): pure planners over beacons, todo items and completion timestamps.
...
test('lockTokenKey: schema-safe hash key, stable per path, distinct across paths', () => {
  ...
  assert.equal(lockTokenKey('f:/a.cs'), lockTokenKey('f:/a.cs'), 'all panes must derive one key per path');
  assert.notEqual(lockTokenKey('f:/a.cs'), lockTokenKey('f:/b.cs'));
...
test('acquire/release tokens: hash keys, acquire carries paneId|path, release clears with null', () => {
  ...
  assert.deepEqual(Object.keys(acq).sort(), [lockTokenKey('f:/a.cs'), lockTokenKey('f:/b.cs')].sort());
  assert.equal(acq[lockTokenKey('f:/a.cs')], 'pZ|f:/a.cs');
...
test('planWriteGuard: soft mode (default) → warn naming path/holders and both lookups, tool passes', () => {
  ...
  assert.deepEqual(g.paths, [LOCKED], 'the normalized path is what matches the token');
  assert.match(g.warning, /pA/);
  assert.match(g.warning, /a\.cs/i);
  assert.match(g.warning, /conflict|locked/i);
  assert.match(g.warning, /by pane pA /, 'single holder is singular');
  assert.match(g.warning, /\/locks/, 'human view');
  assert.match(g.warning, /herdr agent list/, 'agent-readable view');
});
...
test('planWriteGuard (bash): conflicting bash write warns even under hard lock, write tool still blocks', () => {
  ...
  const bash = guard({
    ...
    agents,
    ownPaneId: 'pB',
    hard: true, // hard mode cannot block shell writes: unparsed shell means false positives are too costly
  });
...
test('exact unique pending match → auto-completed (done edit, tier=exact)', () => {
  ...
  assert.deepEqual(lower.edits, [{ op: 'done', content: D }]);
  assert.equal(lower.items.find((t) => t.content === `${D} 补充`)?.status, 'pending');
});
...
test('blocked items whose blocker matches at any tier → unblocked, blocker cleared (failed never unblocks)', () => {
  ...
  assert.equal(sub.edits.filter((e) => e.op === 'unblock').length, 1);
  assert.equal(sub.items[0].status, 'pending');
...
test('completed/abandoned never match; whitespace and case are normalized before exact match', () => {
  ...
  assert.equal(terminal.noteLines.length, 0, 'no candidates → no edits, no notes');
