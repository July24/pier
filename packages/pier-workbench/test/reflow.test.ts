/**
 * Heat reflow domain tests: hook event parsing, the runReflow dispatch table and gates, and the plugin
 * state file (no env, no socket).
 */
...
import {
  OLD_PANE, OLD_TAB, REFLOW_LAYOUT, callRatioOps, calledRpc, makeReflowDeps, piTabIds, type ReflowDepsOpts,
} from './heat-fixtures.ts';
...
test('runReflow：pane.focused 全链——零 swap 原地分级 + 状态回写', async () => {
  const h = makeReflowDeps({ state: { tabs: {}, panes: { 'pane-a': OLD_PANE, 'pane-b': OLD_PANE }, debounce: null } });
...
test('runReflow：pane.created → 记账龄 + 数量重排（D95；首次创建也重排）', async () => {
  const h = makeReflowDeps({ ev: { hook: 'pane.created', paneId: 'pane-new' } });
  await runReflow(h.deps);
...
test('runReflow：pane.closed（D95）→ 按 pane→tab 映射触发数量重排', async () => {
  const h = makeReflowDeps({ ev: { hook: 'pane.closed', paneId: 'pane-c', tabId: null, cause: null },
...
test('runReflow：焦点年龄闸（F1 白名单 / d90 修正）', async (t) => {
  await t.test('幼龄 pane 无 cause → 跳过（3s 白名单）', async () => {
    const h = makeReflowDeps({ ev: { cause: null, paneId: 'pane-a' }, state: { tabs: {}, panes: { 'pane-a': { createdAt: Date.now() } }, debounce: null } });
    await runReflow(h.deps);
    assert.equal(h.calls.length, 0, '不发 layout.export');
  });
  await t.test('不在账的老 pane → 直接接受，不再现编 createdAt 误判新生', async () => {
    const h = makeReflowDeps({ ev: { cause: null, paneId: 'pane-old' } });
    await runReflow(h.deps);
...
test('runReflow：agent_status_changed（档3 语义桥）', async (t) => {
  await t.test('焦点沿用 lastFocus + 原地分级', async () => {
    const h = makeReflowDeps({ ev: { hook: 'pane.agent_status_changed', paneId: 'pane-c', cause: null },
  ...
  await t.test('无 lastFocus 的 count/status 重排 → 退化 first pane 为焦点', async () => {
    const h = makeReflowDeps({ ev: { hook: 'pane.agent_status_changed', paneId: 'pane-b', cause: null },
      state: { tabs: { 'tab-1': { enabled: true } }, panes: {}, debounce: null }, listAgentStatuses: async () => ({}) });
    await runReflow(h.deps);
...
const GATE_CASES: GateCase[] = [
  ...
  { name: 'status 路径受闸：非 pi tab 的 blocked 不触发重排', ratio: false, msg: '非 pi tab 的 blocked 不触发重排',
    opts: { ...piTabIds('tab-other'), ev: { hook: 'pane.agent_status_changed', paneId: 'pane-c', cause: null }, state: { tabs: { 'tab-old': OLD_TAB }, panes: {}, debounce: null }, listAgentStatuses: async () => ({ 'pane-c': 'blocked' }) } },
  { name: 'created 路径受闸：非 pi tab 的数量变化不触发重排', ratio: false, msg: '非 pi tab 的数量变化不触发重排',
    opts: { ...piTabIds('tab-other'), ev: { hook: 'pane.created', paneId: 'pane-new' } } },
];

test('runReflow：收紧闸——只有含 pi pane 的 tab 参与重排', async (t) => {
  for (const c of GATE_CASES) {
    await t.test(c.name, async () => {
      const h = makeReflowDeps(c.opts);
      await runReflow(h.deps);
...
test('runReflow：粘性 pi tab——全部 pi 退回 shell 后记账 tab 仍 reflow（退出命令行仍可焦点放大）', async () => {
  ...
  const holder = makeReflowDeps(bound);
  ...
  const after = makeReflowDeps({ ...bound, ev: { paneId: 'pane-c' } });
  ...
  const opted = makeReflowDeps(bound);
...
test('runReflow：status 在同 pane 集合上手拖 hold；created 重建基线', async () => {
  ...
  const status = makeReflowDeps({ exported: dragged, ev: { hook: 'pane.agent_status_changed', paneId: 'pane-c', cause: null },
  ...
  const created = makeReflowDeps({ exported: dragged, ev: { hook: 'pane.created', paneId: 'pane-new' },
